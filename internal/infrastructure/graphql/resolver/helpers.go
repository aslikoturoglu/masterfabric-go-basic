...
// flowstateFixedEventToModel converts a flowstate fixed event DTO to the GraphQL model.
func flowstateFixedEventToModel(resp *flowstateDTO.FixedEventResponse) *model.FlowstateFixedEvent {
	id, _ := uuid.Parse(resp.ID)
	userID, _ := uuid.Parse(resp.UserID)
	createdAt, _ := time.Parse(time.RFC3339, resp.CreatedAt)
	updatedAt, _ := time.Parse(time.RFC3339, resp.UpdatedAt)
	return &model.FlowstateFixedEvent{
		ID:         id,
		UserID:     userID,
		Title:      resp.Title,
		...
		Category:   model.FlowstateEventCategory(resp.Category),
		CreatedAt:  createdAt,
		UpdatedAt:  updatedAt,
	}
}

func flowstateFixedEventRespToModel(resp *flowstateDTO.FixedEventResponse) *model.FlowstateFixedEvent {
	return flowstateFixedEventToModel(resp)
}

func flowstateFlexibleTaskToModel(resp *flowstateDTO.FlexibleTaskResponse) *model.FlowstateFlexibleTask {
	id, _ := uuid.Parse(resp.ID)
	userID, _ := uuid.Parse(resp.UserID)
	createdAt, _ := time.Parse(time.RFC3339, resp.CreatedAt)
	updatedAt, _ := time.Parse(time.RFC3339, resp.UpdatedAt)
	...
	if resp.Constraints != nil {
		b, _ := json.Marshal(resp.Constraints)
		constraintsStr = string(b)
	}
	return &model.FlowstateFlexibleTask{
		ID:               id,
		UserID:           userID,
		Title:            resp.Title,
		DurationMinutes:  resp.DurationMinutes,
		FrequencyPerWeek: resp.FrequencyPerWeek,
		Priority:         model.FlowstateTaskPriority(resp.Priority),
		PreferredContext: model.FlowstatePreferredContext(resp.PreferredContext),
...
func flowstateFlexibleTaskRespToModel(resp *flowstateDTO.FlexibleTaskResponse) *model.FlowstateFlexibleTask {
	return flowstateFlexibleTaskToModel(resp)
}

func flowstateScheduleToModel(resp *flowstateDTO.ScheduleResponse) *model.FlowstateSchedulePayload {
	scheduleData := "{}"
	if resp.ScheduleData != nil {
		b, _ := json.Marshal(resp.ScheduleData)
		scheduleData = string(b)
	}
	return &model.FlowstateSchedulePayload{
		WeekIdentifier: resp.WeekIdentifier,
